...
import * as fetchMultipart from "./parsers/fetch-multipart.ts";
...
const benchmarks = [
  ...
  { id: "1-large-file", name: "1 large file", message: messages.oneLargeFile },
  {
    ...
    message: messages.oneHundredSmallFiles,
  },
  {
    id: "5-large-files",
    name: "5 large files",
    message: messages.fiveLargeFiles,
  },
  {
...
interface Parser {
  parse(message: messages.MultipartMessage): number | Promise<number>;
}
...
async function runParserBenchmarks(
  ...
  for (let benchmark of benchmarks) {
    if (
      ...
      benchmark.id !== options.benchmarkId
    ) {
      continue;
    }
...
function parseArgOptions(): ParsedOptions {
  ...
  let times =
    timesArg === undefined ? undefined : Number.parseInt(timesArg, 10);

  if (
    benchmarkId !== undefined &&
    !benchmarks.some((benchmark) => benchmark.id === benchmarkId)
  ) {
  ...
  if (times !== undefined && (!Number.isFinite(times) || times <= 0)) {
    throw new Error(
      `Invalid iterations "${timesArg}". Expected a positive integer`,
    );
  }
  ...
  for (let arg of optionArgs) {
    ...
    if (arg.startsWith("--warmup=")) {
      let parsedWarmupTimes = Number.parseInt(
...
async function runBenchmarks(
  ...
  if (parserName === "fetch-multipart" || parserName === undefined) {
    results["fetch-multipart"] = await runParserBenchmarks(
      fetchMultipart,
      options,
    );
  }
  if (parserName === "multipart-parser" || parserName === undefined) {
    results["@remix-run/multipart-parser"] = await runParserBenchmarks(
      multipartParser,
      options,
    );
  }
  ...
  if (parserName === "fastify-busboy" || parserName === undefined) {
    results["@fastify/busboy"] = await runParserBenchmarks(
      fastifyBusboy,
      options,
    );
  }
...
function printResults(results: BenchmarkResults) {
  ...
  for (let [parserName, resultByBenchmark] of Object.entries(results)) {
    summaryResults[parserName] = Object.fromEntries(
      Object.entries(resultByBenchmark).map(([benchmarkName, result]) => [
        benchmarkName,
        result.summary,
      ]),
    );
  ...
  let hasStats = Object.values(results).some((resultByBenchmark) =>
    Object.values(resultByBenchmark).some(
      (result) => result.stats !== undefined,
    ),
  );
  ...
  if (hasStats) {
    ...
    for (let [parserName, resultByBenchmark] of Object.entries(results)) {
      let metricsByBenchmark: Record<string, string> = {};
      for (let [benchmarkName, { stats }] of Object.entries(
        resultByBenchmark,
      )) {
        if (!stats) continue;
        metricsByBenchmark[benchmarkName] =
      ...
      metricResults[parserName] = metricsByBenchmark;
    }
